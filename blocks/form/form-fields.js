...
function setCommonAttributes(field, fd) {
  ...
  if (fd.Maxlength) field.setAttribute('maxlength', fd.Maxlength);
  if (fd.Minlength) field.setAttribute('minlength', fd.Minlength);
...
const createConfirmation = (fd, form) => {
  try {
    ...
    const url = fd.Value.startsWith('http') ? new URL(fd.Value) : new URL(fd.Value, window.location.origin);
    form.dataset.confirmation = url.pathname;
  } catch (e) {
...
const createSubmit = (fd, form) => {
  ...
  button.textContent = fd.Label || fd.Value || fd.Name;
  button.classList.add('button');
  button.type = 'submit';

  // If submit has "inline" style, apply it to the form
  if (fd.Style && fd.Style.toLowerCase().includes('inline')) {
